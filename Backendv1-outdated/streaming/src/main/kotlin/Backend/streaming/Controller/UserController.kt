...
class UserController( val userService: UserService,val actionService: ActionService , val jwtService: JWTService) {
    ...
    fun createAccount(@Valid @RequestBody userCreateDTO: UserCreateDTO): ResponseEntity<Any>{
        return try {
        val createdAccount = userService.createAccount(userCreateDTO); val token = createdAccount.email.let { jwtService.generateToken(email = it, username = createdAccount.user!!.username) }
    ...
    fun deleteAccount(): ResponseEntity<Any>{
        ...
        val (username, email) = AuthUtil.getAuthenticatedInfo()
        return try {
...
//        val users = userPrivateRepository.findAll()
//        return  ResponseEntity.ok(users)
//    }
}